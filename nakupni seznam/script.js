document.addEventListener('DOMContentLoaded', function() {
    ...
    const pocetInput = document.getElementById('pocetInput');
    const saveButton = document.getElementById('saveButton');
    const notesList = document.getElementById('notesList');
    
    saveButton.addEventListener('click', function() {
      const noteText = noteInput.value.trim();
      const pocet= pocetInput.value.trim();
      if (noteText !== '') {
        saveNoteToLocalStorage(noteText, pocet);
        renderNotes();
        noteInput.value = '';
      } else {
    ...
    function saveNoteToLocalStorage(noteText, pocet) {
      const notes = JSON.parse(localStorage.getItem('notes')) || [];
    
      notes.push({ id: generateId(), text: noteText, pocet: pocet });
